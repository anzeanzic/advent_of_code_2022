...
lineReader.on('close', function() { // 34588563455325
	...
	while (!monkeys.has('root')) {
		operations.forEach((operation, i) => {
			if (monkeys.has(operation.monkey1)) {
				const monkey1Val = monkeys.get(operation.monkey1);
				operation.monkey1 = monkey1Val;
			}
			if (monkeys.has(operation.monkey2)) {
				const monkey2val = monkeys.get(operation.monkey2);
				operation.monkey2 = monkey2val;
			}

			operations.forEach((op) => {
				if (op.monkey_name === operation.monkey1) {
					operation.monkey1 = '(' + op.monkey1 + ' ' + op.operator + ' ' + op.monkey2 + ')';
				}
				if (op.monkey_name === operation.monkey2) {
					operation.monkey2 = '(' + op.monkey1 + ' ' + op.operator + ' ' + op.monkey2 + ')';
				}
			});
		});
...
const HandleLines = (line) => {
	...
	if (line_arr[1].indexOf('+') >= 0) {
		...
		allInput.set(monkeyName, line_arr[1].split(' '));
	}
};