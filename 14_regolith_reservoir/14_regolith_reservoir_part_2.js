...
const PrepareMesh = () => {
	...
	const mesh_height = limits.floor - limits.min_y + 1;
...
const DrawSand = () => {
	...
	while (true) {
		...
		if (sand_coor.x > limits.max_x - limits.min_y + 1) {
			//ExtendMesh(false);
		}
...
const ExtendMesh = (toLeft) => {
	for (let i = 0; i < Mesh.length; i++) {
		if (toLeft) {
			Mesh[i] = [i === Mesh.length - 1 ? '#' : '.', ...Mesh[i]];
			limits.min_x--;
		}
		else {
			Mesh[i] = [...Mesh[i], i === Mesh.length - 1 ? '#' : '.'];
			limits.max_x++;
		}
	}
};